...
namespace OurApp.Core.Models
{
    public class JobPosting
    {
        ...
        // Class-based relationship (no FK in code per spec), CompanyId kept for repository queries
        ...
        public string DescriptionDisplay    => !string.IsNullOrWhiteSpace(JobDescription) ? $"Description: {JobDescription}" : null;
        ...
        public string SkillsDisplay         => RequiredSkills.Count > 0
            ? string.Join(", ", RequiredSkills.Select(s => $"{s.Skill?.SkillName} ({s.RequiredPercentage}%)"))
            : "No skills";
    }
}