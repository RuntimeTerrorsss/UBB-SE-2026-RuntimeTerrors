    ...
    public class ApplicantRepository : IApplicantRepository
    {
        public Applicant GetApplicantById(int applicantId)
        {
            ...
            using (var conn = DbConnectionHelper.GetConnection())
            {
                conn.Open();
                string sql = @"
                    SELECT a.applicant_id, a.cv_file_url, a.app_test_grade, a.cv_grade, 
        ...
        public void AddApplicant(Applicant applicant)
        {
            using (var conn = DbConnectionHelper.GetConnection())
            {
                ...
                string sql = @"
                    INSERT INTO applicants (applicant_id, job_id, cv_file_url, app_test_grade, cv_grade, 
                                          company_test_grade, interview_grade, application_status, 
                                          recommended_from_company_id, applied_at, user_id)
                    VALUES (@appId, @jobId, @cv, @appGrade, @cvGrade, @compGrade, @intGrade, @status, @recId, @applied, @userId)";
                ...
                using (var cmd = new SqlCommand(sql, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@cv", string.IsNullOrEmpty(applicant.CvFileUrl) ? DBNull.Value : applicant.CvFileUrl);
        ...
        private Applicant MapReaderToApplicant(SqlDataReader reader)
        {
            var applicant = new Applicant
            {
                ...
                CvFileUrl = reader.IsDBNull(reader.GetOrdinal("cv_file_url")) ? null : reader.GetString(reader.GetOrdinal("cv_file_url")),
